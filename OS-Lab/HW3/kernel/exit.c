...
//release todo_list memory - modified
static inline void __exit_TODO(struct task_struct * tsk)
{
    if(tsk->punish_timer != NULL) {
        del_timer_sync(tsk->punish_timer);
        kfree(tsk->punish_timer);
        tsk->punish_timer = NULL;
    }
    if(list_empty(&(current->firstTodo)))  return 0;
    list_t *iter_todo_list;
    list_for_each(iter_todo_list, &(tsk->firstTodo)) { //freeing nodes' memory
        todo_node *cur_node = list_entry(iter_todo_list, todo_node, todo_list);
        kfree(cur_node->description);
        kfree(cur_node);
    }
    return 0;
}

void exit_TODO(struct task_struct *tsk)
{
    __exit_TODO(tsk);
}
