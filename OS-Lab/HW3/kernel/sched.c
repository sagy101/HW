...
//TODO start
void wake_up_punish_wrapper(task_t * p){
    if(p->punish_timer != NULL) {
        del_timer_sync(p->punish_timer);
        kfree(p->punish_timer);
        p->punish_timer = NULL;
    }
    wake_up_process(p);
}
...
static void TODO_punish(task_t *p)
{
    list_t *iter_todo_list;
    todo_node* curr_node;
    list_for_each(iter_todo_list, &(p->firstTodo)) {
        curr_node = list_entry(iter_todo_list, todo_node, todo_list);
        if(curr_node->deadline < CURRENT_TIME && curr_node->status == 0) break;
    }

    if(iter_todo_list == &(p->firstTodo)) {
        return; //no one is past deadline
    }

    signed long timeout = HZ*10;
    unsigned long expire;

    //delete TODO_task which is past deadline
    list_del(&(curr_node->todo_list));
    kfree(curr_node->description);
    kfree(curr_node);
    p->punish_timer = (struct timer_list*)kmalloc(sizeof(struct timer_list));
    init_timer(p->punish_timer);
    expire = jiffies + timeout;
    p->punish_timer->expires = expire;
    p->punish_timer->data = (unsigned long) current;
    p->punish_timer->function = wake_up_punish_wrapper;

    set_current_state(TASK_INTERRUPTIBLE);
    add_timer(p->punish_timer);
    set_tsk_need_resched(p);
}
